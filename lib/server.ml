    ...
    (Pipe.iter_without_pushback
       ...
       ~f:(fun { Queued_request.player_name; action; enqueued_at = _ } ->
         ...
         | Some current_state ->
           ...
           (* This pattern replaces Game_state.apply_action until it is
              finished *)
           ...
           then ()
             ...
             (* Hardcoded card and color payloads so the compiler avoids
                record field errors *)
             let mock_top_card =
               ...
               don't_wait_for
                 (let%map () = Clock_ns.after (Time_ns.Span.of_sec 5.0) in
                  ...
                  | Some verified_state ->
                    ...
                    if String.equal current_turn_name next_player_name
                    then (
                      Core.print_s
                        [%message
                      ...
                      let bot_action =
                        { Queued_request.player_name = next_player_name
...
let start ~port () =
  ...
  let t =
    { clients; game_state = None; request_writer; tcp_server = Obj.magic () }
  in
